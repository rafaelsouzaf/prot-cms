 ...
 * </dl>
 */
public class ProtInfo extends ContentPanel {

	private static Stack<ProtInfo> infoStack = new Stack<ProtInfo>();
	private static ArrayList<ProtInfo> slots = new ArrayList<ProtInfo>();
	...
	private static ProtInfo pop() {
		ProtInfo info = infoStack.size() > 0 ? (ProtInfo) infoStack.pop() : null;
		if (info == null) {
			info = new ProtInfo();
		}
		return info;
	...
	private static void push(ProtInfo info) {
		infoStack.push(info);
	}
	...
	protected Point position() {
		...
		int left = s.width - config.width - 50 + XDOM.getBodyScrollLeft();
		int top = s.height - config.height - 10 - (level * (config.height + 10)) + XDOM.getBodyScrollTop();
